...
- **Transparência no Windows:** o `Clipboard.GetImage()` do .NET usa um caminho de bitmap que não preserva canal alfa (fundo transparente vira preto). O helper do Windows agora tenta primeiro ler os bytes crus do formato `"PNG"` que o app de origem também costuma colocar na clipboard — que preserva a transparência corretamente — e só cai para o `Bitmap` genérico (sem alpha) como último recurso.
